...
void onMouse(int event, int x, int y, int flags, void *param)
{
	...
	if (event == CV_EVENT_LBUTTONDOWN) {
		...
		cv::imshow(windowName, matSrc);
		cut = matInit(rectSelect);
		cv::imwrite("D:/USTC/Image analyze/ppt/image/result_cut.jpg", cut);
		cv::Mat matSource;
		matSource = cv::imread("D:/USTC/Image analyze/ppt/image/source.jpg");
		cut2 = matSource(rectSelect);
		cv::imwrite("D:/USTC/Image analyze/ppt/image/result_cut2.jpg", cut2);
		haveselect = true;
...
int main()
{
	...
	matInit = cv::imread("D:/USTC/Image analyze/ppt/image/result.jpg");
	...
	//img = cv::imread("D:/USTC/Image analyze/ppt/image/result.jpg");
	//matSrc = img;

	//cv::namedWindow(windowName, CV_WINDOW_NORMAL);
	while (haveselect == false) {
	...
	//cutRect.x = 40;
	//cutRect.y = 100;
	//cutRect.width = img.cols - 40;
	//cutRect.height = img.rows - 100;



	img = cv::imread("D:/USTC/Image analyze/ppt/image/result_cut.jpg", CV_LOAD_IMAGE_GRAYSCALE);
