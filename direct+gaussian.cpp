...
void onMouse(int event, int x, int y, int flags, void *param)
{
	...
	if (event == CV_EVENT_LBUTTONDOWN) {
		...
		if (haveselect == false) {
			...
			cv::rectangle(matSrc, ptfixs, ptfixe, cv::Scalar(0, 0, 0xFF), thickness, lineType);
			...
			cv::rectangle(matSrc, ptCurrent, ptAuto, cv::Scalar(0, 0xFF, 0), thickness, lineType);
			cv::imshow(windowName, matSrc);
...
int main()
{
	...
	while (1) {
		...
		//简单的覆盖
		matInit = cv::imread("D:/project/opencv/vs/test/image/result.jpg");
		Mat imageROI = matInit(rectSelect);
		Mat mask;
		cvtColor(matChange2, mask, CV_BGR2GRAY);
		//Mat element = getStructuringElement(MORPH_RECT, Size(15, 15));
		//Mat matChange2_dilate;
		//dilate(matChange2, matChange2_dilate, element);

		matChange2.copyTo(imageROI, mask);
		imwrite("D:/project/opencv/vs/test/image/result_without_Gaussian.jpg", matInit);

		cv::Rect A, B, C, D;
		A.x = rectSelect.x - 3;
		A.y = rectSelect.y - 3;
		A.width = rectSelect.width + 6;
		A.height = 6;
		B.x = rectSelect.x - 3;
		B.y = rectSelect.y + rectSelect.height- 3;
		B.width = rectSelect.width + 6;
		B.height = 6;

		C.x = rectSelect.x - 3;
		C.y = rectSelect.y - 3;
		C.width = 6;
		C.height = rectSelect.height +6;
		D.x = rectSelect.x + rectSelect.width - 3;
		D.y = rectSelect.y  - 3;
		D.width = 6;
		D.height = rectSelect.height + 6;

		cv::GaussianBlur(matInit(A), matInit(A), Size(5, 5), 0, 0);
		cv::GaussianBlur(matInit(B), matInit(B), Size(5, 5), 0, 0);
		cv::GaussianBlur(matInit(C), matInit(C), Size(5, 5), 0, 0);
		cv::GaussianBlur(matInit(D), matInit(D), Size(5, 5), 0, 0);
		cv::GaussianBlur(matInit(A), matInit(A), Size(5, 5), 0, 0);
		cv::GaussianBlur(matInit(B), matInit(B), Size(5, 5), 0, 0);
		cv::GaussianBlur(matInit(C), matInit(C), Size(5, 5), 0, 0);
		cv::GaussianBlur(matInit(D), matInit(D), Size(5, 5), 0, 0);
		...
		cv::GaussianBlur(matInit(C), matInit(C), Size(5, 5), 0, 0);
		cv::GaussianBlur(matInit(D), matInit(D), Size(5, 5), 0, 0);
		imwrite("D:/project/opencv/vs/test/image/result_Gaussian.jpg", matInit);
		/*cv::imshow(windowName, matSrc);
		cv::waitKey(2000);
		...
		//Mat src_mask = Mat::zeros(matChange2.rows, matChange2.cols, matChange2.depth());
		//Point poly[1][4];
		//poly[0][0] = Point(0, 0);
		...
		//		}
		//		/*	else if (comp.at<Vec3b>(row, col).val[2] < 40 && comp.at<Vec3b>(row, col).val[2] >= 20) {
		//		comp.at<Vec3b>(row, col).val[2] += 100;
		//		}
		//		else if (comp.at<Vec3b>(row, col).val[2] < 60  &&  comp.at<Vec3b>(row, col).val[2] >= 40) {
		//		comp.at<Vec3b>(row, col).val[2] += 75;
		//		}
		//		else if (comp.at<Vec3b>(row, col).val[2] < 80 && comp.at<Vec3b>(row, col).val[2] >= 60) {
		//		comp.at<Vec3b>(row, col).val[2] += 50;
		//		}
		//		else if (comp.at<Vec3b>(row, col).val[2] < 100 && comp.at<Vec3b>(row, col).val[2] >= 80) {
		...
		////cv::GaussianBlur(matInit(rectChange), matInit(rectChange), Size(5, 5), 0, 0);
		...
		////cv::GaussianBlur(matInit(rectChange2), matInit(rectChange2), Size(15, 15), 0, 0);

		//seamlessClone(matChange2, output, src_mask, center, output, NORMAL_CLONE);

		namedWindow("dilate + poisson", CV_WINDOW_NORMAL);
		imshow("dilate + poisson", matInit);
		imwrite("D:/project/opencv/vs/test/image/dilate_poisson.jpg", matInit);
		...
		cv::imshow(windowName, matInit);
		cv::waitKey(2000);
		imwrite("D:/project/opencv/vs/test/image/result.jpg", matInit);
		haveselect2 = false;
		haveselect = false;
